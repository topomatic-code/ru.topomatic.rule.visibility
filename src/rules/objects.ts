...
/**
 * Вычисляет центр масс 3D-моделей на основе их мешей и вершин.
 * Результат записывается в переданный вектор `target`.
 *
 * @param target - Вектор, в который будет записан центр модели.
 * @param model - 3D-модель, для которой вычисляется центр.
 * @returns `true`, если центр был успешно вычислен; `false`, если модель пуста.
 */
function modelsCenter(target: vec3, models: DwgModel3d[]): boolean {
    let x = 0;
    let y = 0;
    let z = 0;
    let n = 0;
    const mc: vec3 = [0.0, 0.0, 0.0];
    for (const model of models) {
        if (modelCenter(mc, model)) {
            x += mc[0];
            y += mc[1];
            z += mc[2];
            ++n;
...
function activateDiagnostic(diagnostic: Diagnostic, active: boolean) {
    ...
    const { ctx, alignment, objects, rule, rangeStart, rangeEnd, obstacles } = visibilityDiagnostic;

    const cadview = ctx.cadview;
    if (!cadview) return;

    const objs = objects.map(item => item.deref()).filter(item => item !== undefined);
    if (!objs) return;

    const objectCenter: vec3 = [0, 0, 0];
    if (!modelsCenter(objectCenter, objs)) return;
    ...
    cadview.layer.selectObjects(
        (obj) => obstacles.has(obj) || obj === alignment || set.has(obj),
        true
    );
    ...
    'rule:visibility:objects': (ctx: Context): DiagnosticRule<ObjectsVisibilityRule> => {
        return {
            ...
            async execute(app: Application, rule: ObjectsVisibilityRule, diagnostics: DiagnosticCollection, _progress: WorkerProgress) {
                ...
                const layers = new Map<String, DwgModel3d[]>();
                for (const object of objects) {
                    const layerId = object.layer?.key ?? '0';
                    const collection = layers.get(layerId);
                    if (collection === undefined) {
                        layers.set(layerId, [object]);
                    } else {
                ...
                for (const object of layers.values()) {
                    const ignore = new Set<DwgModel3d>(object);
                    for (const alignment of alignments) {
                        if (!modelsCenter(objectCenter, object)) {
                            continue;
                        }
                        ...
                        for (let station = 0; station <= length; station += rule.viewPointStep) {
                            ...
                            if (!outOfView) {
                                ...
                                obstacles = drawingLayer.selectableObjects(frustum, (obj) => {
                                    if (ignore.has(obj) || obj.type !== DwgType.model3d) {
                                        return false;
                                    }
                            ...
                            if (outOfView || obstacle!.done) {
                                if (rangeStart >= 0) {
                                    collection.push({
                                        ...
                                        source: `${alignment.source}, ${object[0].layer?.layer?.name}/${object[0].layer?.name}`,
                                        tooltip: ctx.tr('Препятствия мешают видимости'),
                                        activation: activateDiagnostic,

                                        ctx,
                                        alignment,
                                        objects: object.map(item => new WeakRef(item)),
                                        rule,
                                        rangeStart,
                                        rangeEnd,
